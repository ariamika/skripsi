...
<head>
    <title>Yadabagasket - Daftar</title>
    <!-- for-mobile-apps -->
...
<body>
            ...
            <div class="login-form-grids">
                ...
                <form method="post">
                    ...
                    if ($err_province) {
                        ...
                        $province = json_decode($response_province, true);
                    }
                    
                    ?>
                    <select id="provinsi" class="form-control" style="margin: 1em 0;" name="provinsi"
                        onchange="getCity()" required>
                        <option value="">-Pilih Provinsi-</option>
                        <?php
                        foreach ($province['rajaongkir']['results'] as $key => $value) {
                            echo "<option value='" . $value['province_id'] . "'>" . $value['province'] . '</option>';
                        }
                        ?>
                    </select>

                    <select id="kota" class="form-control" style="margin: 1em 0;" name="kota" required>
                    </select>
    ...
    <script type="text/javascript">
        function getCity() {
            const province_id = $("#provinsi").val();
            $("#kota").html("");
            if (province_id !== "") {
                // API untuk menampilkan dan mengisi select option kota sesuai dengan provinsi yang dipilih
                $.ajax({
                    type: "POST",
                    url: 'api/city.php',
                    data: {
                    ...
                    success: function(data) {
                        ...
                        let optionCity = `<option value="">-Pilih Kota-</option>`;
                        for (let i = 0; i < city.length; i++) {
                            optionCity += `<option value="${city[i].city_id}">${city[i].city_name}</option>`;
                        }
                        $("#kota").show();
                        $("#kota").html(optionCity);
                    }
                });
            } else {
                $("#kota").hide();
        ...
        $(document).ready(function() {
            ...
            console.log("Provinsi : ", $("#provinsi").val());
